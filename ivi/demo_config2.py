...
class ConfigureInputDevice():
    ...
    def configure_touch_for_android2(self):
        ...
        logging.info('Starting to detecting touch devices and touch information from input devices...')
        input_device_list = self.get_input_device_list()
        display_info_list = self.get_display_info_list()
        new_display_info_list = self.refactor_display_info_list(input_device_list, display_info_list)
        if new_display_info_list:
        ...
        for display_info in new_display_info_list:
            if 'touch_device' in display_info and 'vendor' in display_info['touch_device'] \
                and 'product' in display_info['touch_device']:
                new_vendor_product_list.append([display_info['touch_device']['vendor'], display_info['touch_device']['product']])
        index = 0
        for display_info in new_display_info_list:
            if 'touch_device' not in display_info or not display_info['touch_device']:
                continue
            index += 1
            print('-'*20+'{:-<{width}}'.format('Step 5.4.{}. detect the touch screen for Display {}'.format(index, display_info['uniqueId']), width=80))
            logging.info('Detected Touch device: ')
            logging.info('    Name: {}  displayId: {}  uniqueId: {}'.format(display_info['touch_device']['Name'], display_info['displayId'], display_info['uniqueId']))
            logging.info('    Event: {}  Location: {}'.format(display_info['touch_device']['Path'], display_info['touch_device']['Location']))
            logging.info('    Identifier: {}'.format(display_info['touch_device']['Identifier']))
            logging.info('Starting to create idc file for it...')
            num_vp = 0
            for vp in new_vendor_product_list:
                if vp[0] == display_info['touch_device']['vendor'] and vp[1] == display_info['touch_device']['product']:
                    num_vp += 1
            if num_vp <= 1:    
                logging.debug('No duplicated vendor+product ids for touch devices.')
                idc_file_name = os.path.join(self._outputdir,  
                    'Vendor_{}_Product_{}.idc'.format(display_info['touch_device']['vendor'], display_info['touch_device']['product']))
                ...
                name = display_info['touch_device']['Location'].strip()
                char_list = '/,:.'
                for char in char_list:
                    name = name.replace(char, '_')
                ...
                if display_info['uniqueId'] in output:
                    logging.debug(output)
                    logging.info('Creating idc file for Touch Display {} successfully.'.format(display_info['uniqueId']))
                else:
                    logging.error('Creating idc file for Touch Display {} failed.'.format(display_info['uniqueId']))
                    logging.error(output) if output else ''
                    logging.error(err) if err else ''
    ...
    def configure_touch_for_android(self):
        ...
        for display_info in display_info_list:
            ...
            for i in range(TOUCH_DETECT_RETRY_TIMES):
                ...
                if choice == 'n':
                    logging.info('You selected n, ignore touch config for current Display.')
    ...
    def refactor_display_info_list(self, input_device_list, display_info_list):
        new_display_info_list = []
        for display_info in display_info_list:
            if 'DeviceType' not in display_info or 'TOUCH' not in display_info['DeviceType']:
               continue
            if 'devices' in display_info and display_info['devices']:
                for d in display_info['devices']:
                    did = None
                    try:
                        did=int(d)
                    except Exception as e:
                        logging.debug('device with id {} excpetion: {}'.format(d, str(e)))
                    else:
                        for input_device in input_device_list:
                            if str(did) == str(input_device['ID']) and 'TOUCH | TOUCH_MT | EXTERNAL' in input_device['Classes']:
                                display_info['touch_device'] = input_device
                                new_display_info_list.append(display_info)
                                break
        logging.debug('new_display_info_list:')                        
        logging.debug(new_display_info_list)                        
        return new_display_info_list                        
...
if __name__ == "__main__":
    ...
    if not configed:
        ...
        #config.configure_touch_for_android2()
